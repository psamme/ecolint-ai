...
export const DEFAULT_CONFIG: Required<EcoLintFileConfig> = {
  minSeverity: "low",
  ignoredRules: [],
  ignoredPaths: [],
  provider: "unknown",
...
export const CONFIG_FILENAME = "ecolint.config.json";

/** Contents written by `ecolint-ai init`. */
...
/**
 * Load `ecolint.config.json`, searching the scan path first and then the current
 * working directory. Returns an empty object (not defaults) when no file exists,
 * so callers can tell "unset" from "set to default".
 */
export async function loadConfig(scanPath: string): Promise<EcoLintFileConfig> {
  const candidates = [
...
function normalizeConfig(input: unknown, source: string): EcoLintFileConfig {
  if (typeof input !== "object" || input === null) {
    throw new Error("expected a JSON object");
  }
  const obj = input as Record<string, unknown>;
  const config: EcoLintFileConfig = {};
